...
**ALWAYS use the `AskUserQuestion` tool when asking the user a question.** This applies to all questions — multiple-choice selections, yes/no decisions, and open-ended requests for feedback. Never present choices as plain-text lists for the user to respond to manually.

- For questions with discrete options (e.g., execution mode, guard decisions): provide the options in the `AskUserQuestion` `options` array so the user gets an interactive selection menu.
- For open-ended questions (e.g., "any feedback on this plan?"): provide options like "Looks good, proceed" and "I have feedback" so the user can quickly approve or indicate they want to give input.
...
1. Check if file `.claude/CLAUDE.local.md` exists. If it does, read it for environment setup instructions and follow them for all subsequent phases. If it does not exist, skip to step 2. **Do NOT parallelize this step with step 2** — complete this check first, then proceed.
...
6. Use `AskUserQuestion` to offer execution mode choice:
   - Option 1: **Full Autopilot** — Execute all phases automatically, no pauses between phases
   - Option 2: **Human-in-the-Loop** — Pause after each major phase for review and feedback
...
7. Create working directory and write the active-review pointer:
   ...
   **Guard — name collision**: If `reviews/[project-short-name]/literature-review-final.md` already exists, use `AskUserQuestion` to warn the user and present options: (1) overwrite the existing review, or (2) choose a different name (e.g., append `-2`).

   **Guard — concurrent review**: If `reviews/.active-review` already exists and points to a *different* directory, use `AskUserQuestion` to warn the user and present options: (1) abandon the previous review and start fresh, or (2) resume the previous review instead.
...
1. Receive and review research idea from user. If you require further information, clarification or direction, use `AskUserQuestion` to ask.
...
5. Use `AskUserQuestion` to get user feedback on the plan (e.g., options: "Looks good, proceed" / "I have feedback"). Iterate if needed using Task tool to invoke `literature-review-planner` agent again
